...
from model import SNN_TextCNN, ANN_TextCNN, Normal_TextCNN, TextCNNEmbeded
import numpy as np
...
def build_model(args: SNNArgs):
    ...
    if args.model_mode == "ann":
        ...
        elif args.model_type == "textcnn_embed":
            args.model = TextCNNEmbeded(args).to(args.device)
...
def ann_train(args: SNNArgs):
    def get_tensor_dataset(file):
        ...
        for i in range(len(sample_list)):
            ...
            text_list = nltk.tokenize(sample_list[i][0])
    ...
    if args.random_tensor == "True":
        with open(f"data/{args.dataset_name}/train_{args.dataset_name}.word2id", 'rb') as f:
...
def conversion(args: SNNArgs):
    if args.conversion_mode == "normalize":
        ...
        if args.conversion_normalize_type == "model_base":
            ...
            # Here we use constant number to do data based normalization
            # As show in "Fast-Classifying, High-Accuracy Spiking Deep Networks Through Weight and Threshold Balancing", it is reasonable.
            output_layer_factor = 1.19   # hyperparameter
            convs_layer_factor = 1.0021  # hyperparameter
            for key in saved_weights.keys():
                # default: fc is output layer
...
def cal_power(args:SNNArgs):
    if args.model_mode == "snn":
        build_surrogate(args=args)
        build_model(args)
        saved_weights = torch.load(args.power_model_path)
        args.model.load_state_dict(saved_weights, strict=False)
        total = sum([param.nelement() for param in args.model.parameters()])
        ...
        power_list = []
        sops_list = []
        for data, targets in args.test_dataloader:
            reset(args.model)
            data = data.to(args.device)
            targets = targets.to(args.device)
            for step in range(args.num_steps):
                flops_of_all_layers, fire_rates_of_all_layers= args.model.cal_flop_and_fire_rate(data.transpose(1, 0)[step])
                sops = np.dot(np.array(flops_of_all_layers), np.array(fire_rates_of_all_layers)) * args.num_steps / 1e9 # G = 1e9
                sops_list.append(sops)
                power_per_sample = 77 * sops * 1e9 * 1e-12 # 1fJ = 1e-15 J = 1e-12 mJ
                power_list.append(power_per_sample)
        print(np.mean(sops_list))
        print(np.mean(power_list))
    elif args.model_mode == "ann":
        build_model(args)
        saved_weights = torch.load(args.power_model_path)
        args.model.load_state_dict(saved_weights, strict=False)
        ...
        for data, targets in args.test_dataloader:
            data = data.to(args.device)
            targets = targets.to(args.device)
            flops_of_all_layers = args.model.cal_flop(data) 
            flops_per_sample = np.sum(np.array(flops_of_all_layers)) / 1e9 # G = 1e9
            flops_list.append(flops_per_sample)
            power_per_sample = 12.5 * flops_per_sample * 1e9 * 1e-9 # 1pJ = 1e-12 J = 1e-9 mJ
            power_list.append(power_per_sample)
